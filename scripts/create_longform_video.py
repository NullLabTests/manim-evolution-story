...
=========================================================================
A long-form Manim educational video with 13 scenes spanning cosmic to human history.

Rendering commands:
  manim -pql create_longform_video.py TitleScene
  manim -pqh create_longform_video.py TitleScene
  manim -pql --save_sections create_longform_video.py TitleScene
  ...etc for each scene

Assembly (after rendering all scenes in 1080p60):
  ffmpeg -f concat -safe 0 -i <(for f in media/videos/create_longform_video/1080p60/*.mp4; do echo "file '$PWD/$f'"; done) -c copy final_video.mp4
"""
...
class AnimatedParticleBurst(Animation):
    """A burst of particles expanding outward from a point."""

    def __init__(self, mobject, num_particles=60, speed=3.0, color=WHITE, **kwargs):
    ...
    def begin(self):
        ...
        for _ in range(self.num_particles):
            ...
            dx = np.sin(theta) * np.cos(angle)
            dy = np.sin(theta) * np.sin(angle)
            dz = np.cos(theta)
            dir_vec = np.array([dx, dy, dz * 0.3])  # flatten slightly for 2D
            vel = np.random.uniform(0.5, 1.0) * self.speed * dir_vec[:2]
            p = Dot(radius=np.random.uniform(0.01, 0.03), color=self.burst_color)
            p.move_to(center)
            p.velocity = vel
    ...
    def clean_up_from_scene(self, scene=None):
        if scene:
            scene.remove(self.particles)

    def interpolate_mobject(self, alpha):
        for p in self.particles:
            p.shift(p.velocity * self.rate_func(alpha) * 0.05)
            p.set_opacity(1 - self.rate_func(alpha))
...
class TitleScene(Scene):
    def construct(self):
        ...
        subtitle_text = "THE EPIC 13.8-BILLION-YEAR STORY OF EVOLUTION"

        subtitle = Text(subtitle_text, font_size=28, color=GREY_B)
        ...
        # Camera pulses gently
        for _ in range(2):
            self.play(
                stars.animate.set_opacity(0.6),
                run_time=1.5,
            )
            self.play(
...
class StarFormationScene(Scene):
    def construct(self):
        ...
        self.play(
            *[GrowFromCenter(n, run_time=np.random.uniform(1, 2)) for n in nebulae],
            run_time=2.0,
        )
        self.wait(0.5)
        ...
        self.play(
            FadeOut(VGroup(title, year_label, nebulae, stars_group, spiral_group, spiral2, elements_text)),
            run_time=1.5,
        )
        self.wait(0.3)
...
class SolarSystemScene(Scene):
    def construct(self):
        ...
        self.play(
            *[Create(o, run_time=0.3) for o in orbits],
            run_time=1.5,
        )

        self.play(
            *[GrowFromCenter(p, run_time=0.4) for p in planets],
            run_time=2.0,
        )
        self.wait(0.5)
        ...
        self.play(
            *[
                UpdateFromAlphaFunc(
                    planet,
                    lambda m, a, p=p, c=sun.get_center(), r=orbit_data[i][1], sp=np.random.uniform(0.3, 0.8): m.move_to(
                        c + [r * np.cos(a * 2 * np.pi * sp), r * np.sin(a * 2 * np.pi * sp), 0]
                    ),
                    run_time=4.0,
                    rate_func=linear,
                )
                for i, (p, planet) in enumerate(zip(orbit_data, planets))
            ],
            run_time=5.0,
        )
        ...
        # Early Earth - molten
        earth_info = Text(
            "Early Earth: molten, bombarded by asteroids",
            font_size=22,
            color=GREY_B,
        )
...
class OriginOfLifeScene(Scene):
    def construct(self):
        ...
        # Chemical building blocks floating
        molecules_text = [
            ("H₂O", BLUE_D),
            ("CH₄", GREY_B),
            ("NH₃", PURPLE),
        ...
        molecules = VGroup()
        for i, (formula, color) in enumerate(molecules_text):
        ...
        self.play(
            *[FadeIn(m, scale=0.3, run_time=np.random.uniform(0.5, 1.2)) for m in molecules],
            run_time=2.0,
        )
        self.wait(0.5)
        ...
        # Flash - formation of first organic compounds
        ...
        # First protocell - simple membrane
        ...
        self.play(
            DrawBorderThenFill(membrane),
            GrowFromCenter(inner),
            run_time=1.5,
        )
        ...
        self.play(
            membrane.animate.shift(LEFT * 0.5),
            membrane2.animate.shift(RIGHT * 0.5),
            run_time=2.0,
        )
        self.wait(0.5)
...
class GreatOxidationScene(Scene):
    def construct(self):
        ...
        self.play(
            *[GrowFromCenter(c, run_time=np.random.uniform(0.3, 0.8)) for c in cyano_group],
            run_time=2.0,
        )
...
class EukaryotesScene(Scene):
    def construct(self):
        ...
        # Another prokaryote being engulfed
        ...
        # Engulfing animation
        self.play(
            prey.animate.move_to(prok.get_center() + RIGHT * 0.1),
            run_time=1.5,
        )

        # The prey becomes an organelle
        engulf_flash = create_glow_circle(0.8, PURPLE, opacity=0.4)
        engulf_flash.move_to(prok.get_center())
        self.play(
            engulf_flash.animate.scale(1.5).set_opacity(0),
            run_time=0.8,
        )
        self.wait(0.3)

        # Transform into eukaryote
        ...
        self.play(
            ReplacementTransform(prok.copy(), euk),
            run_time=0.5,
        )

        self.play(
            Create(arrow),
            FadeIn(nucleus, scale=0.5),
...
class CambrianExplosionScene(Scene):
    def construct(self):
        ...
        self.play(
            *[FadeIn(c, scale=0.3, run_time=np.random.uniform(0.4, 1.0)) for c in creatures],
            run_time=2.5,
        )

        # Labels for some creatures
        creature_names = [
            ("Trilobite", [-4, -3.0, 0]),
        ...
        # Animate creatures swimming
        for i, c in enumerate(creatures):
            self.play(
                c.animate.shift(UP * np.random.uniform(0.1, 0.3) * (-1 if i % 2 == 0 else 1)),
                run_time=0.5,
            )
        ...
        branch_lines = VGroup()
        branch_texts = VGroup()
        for name, pos, color in branch_labels:
        ...
        self.play(
            *[Create(bl) for bl in branch_lines],
            *[Write(bt) for bt in branch_texts],
            run_time=1.5,
        )
...
class SeaToLandScene(Scene):
    def construct(self):
        ...
        self.play(
            FadeIn(land_area, shift=LEFT * 0.5),
            *[Create(s) for s in land],
            run_time=1.0,
        )
        ...
        self.play(
            ReplacementTransform(fish.copy(), tetrapod),
            run_time=1.5,
        )
        ...
        self.play(
            *[Create(p, run_time=0.3) for p in plants],
            run_time=1.0,
        )
        self.wait(0.3)
        ...
        self.play(
            *[Create(d, run_time=0.3) for d in [dino_body, dino_head, dino_neck, dino_tail, dino_leg]],
            run_time=1.2,
        )
        self.wait(2.0)
...
class RiseOfMammalsScene(Scene):
    def construct(self):
        ...
        self.play(
            asteroid.animate.move_to(impact_point),
            run_time=2.0,
        ...
        self.play(
            impact_glow.animate.scale(4).set_opacity(0),
            run_time=1.0,
        )
        self.wait(0.3)
        ...
        self.play(
            dust.animate.scale(2).set_opacity(0.8),
            run_time=0.8,
        )
        self.wait(0.5)
        ...
        self.play(
            FadeOut(dust),
            FadeOut(dino_fade_text),
            run_time=0.8,
        )

        # Small mammals emerge
        ...
        self.play(
            *[FadeIn(m, scale=0.5, run_time=np.random.uniform(0.3, 0.8)) for m in mammals],
            run_time=2.0,
        )
        self.wait(0.8)
        ...
        # Mammal radiation
        radiation_text = Text(
            "Mammals diversify into niches left vacant",
            font_size=18,
            color=GREY_B,
        )
        ...
        self.play(
            *[FadeIn(e, run_time=0.5) for e in examples],
            run_time=1.0,
        )
        self.wait(2.0)
...
class PrimateLineageScene(Scene):
    def construct(self):
        ...
        self.play(
            *[FadeIn(a, shift=LEFT * 0.3, run_time=0.5) for a in adaptations],
            run_time=2.0,
        )
...
class HumanEvolutionScene(Scene):
    def construct(self):
        ...
        self.play(
            *[FadeIn(s, run_time=0.5) for s in species_labels],
            run_time=3.0,
        )
        self.wait(0.5)

        # Brain size increasing
        brain_title = Text("Increasing Brain Size", font_size=18, color=BLUE)
        brain_title.move_to(LEFT * 5 + UP * 1.5)
        ...
        self.play(
            *[FadeIn(b, shift=UP * 0.3, run_time=0.4) for b in brain_bars],
            run_time=2.0,
        )
        ...
        tech_title = Text("Technology Milestones", font_size=18, color=C_FIRE_RED)
        tech_title.move_to(RIGHT * 4 + UP * 1.5)
        ...
        self.play(
            *[FadeIn(t, shift=UP * 0.2, run_time=0.5) for t in tech_group],
            run_time=2.5,
        )
        self.wait(0.5)
        ...
        # Out of Africa
        africa_text = Text(
            "Homo sapiens migrates out of Africa ~70 kya",
            font_size=18,
            color=C_WARM_GOLD,
        )
...
class ConclusionScene(Scene):
    def construct(self):
        ...
        self.play(
            *[FadeIn(e, scale=0.5, run_time=0.3) for e in event_labels],
            run_time=2.0,
        )
        self.wait(1.0)
        ...
        self.play(
            *[FadeIn(m, shift=UP * 0.2, run_time=1.0) for m in message_group[:4]],
            run_time=4.0,
        )
        self.wait(0.5)
        self.play(
            *[FadeIn(m, shift=UP * 0.2, run_time=1.2) for m in message_group[4:]],
            run_time=3.0,
        )
